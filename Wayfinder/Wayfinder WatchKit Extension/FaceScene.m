...
- (instancetype)initWithCoder:(NSCoder *)coder
{
	...
	if (self) {
		...
		self.majorTickHeight = 4;
		self.majorTickWidth = 1;
...
-(void)setupTickmarksForRoundFaceWithLayerName:(NSString *)layerName
{
	...
	for (int i = 0; i < 120; i++)
	{
		...
		SKSpriteNode *tick = [SKSpriteNode spriteNodeWithColor:self.minorMarkColor size:CGSizeMake(0.5, shortTickHeight)];
		...
		{
			if ((self.tickmarkStyle == TickmarkStyleStackMajor) || (self.tickmarkStyle == TickmarkStyleMinor) || i % 10 != 0)
			{
...
-(void)setupColors
{
	...
	switch (self.theme) {
		case ThemeHermesPink:
		{
			...
			faceBackgroundColor = [SKColor colorWithRed:0 green:0 blue:0 alpha:0];
			majorMarkColor = [SKColor colorWithRed:1 green:1 blue:1 alpha:1];
            minorMarkColor = [SKColor colorWithRed:1 green:1 blue:1 alpha:0.5];
            inlayColor = [SKColor colorWithRed:0 green:0 blue:0 alpha:1];
